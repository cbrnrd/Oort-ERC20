var Oort = artifacts.require("./OortToken.sol");
...
contract('Oort', function(accounts){
  ...
  it('initializes the contract with the correct values', function() {
    return Oort.deployed().then(function(instance){
      ...
      assert.equal(name, 'Oort Token', 'has the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol){
      assert.equal(symbol, 'OORT', 'has the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard){
      assert.equal(standard, 'Oort Token v1.0', 'has the correct standard');
    });
  });
  ...
  it('allocates the initial supply upon deployment', function(){
    return Oort.deployed().then(function(instance){
      ...
      assert.equal(totalSupply.toNumber(), 1000000000, 'sets the total initial supply to 1,000,000,000');
  ...
  it('transfers token ownership', function() {
    return Oort.deployed().then(function(instance) {
      ...
      assert(error.message != "", 'error message given: ' + error.message);
